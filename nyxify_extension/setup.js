...
function goToInstaller() {
  ...
  setTimeout(() => {
    window.location.href = SETUP_VIEW;
  }, 600);
}
...
function filterOsBlocks() {
  ...
  const os = plat.indexOf("win") >= 0
    ? "windows"
    : (plat.indexOf("mac") >= 0 ? "macos" : (plat.indexOf("linux") >= 0 ? "linux" : ""));
  if (!os) {
    return;
  }
  document.querySelectorAll("#steps .os").forEach((block) => {
    const h3 = block.querySelector("h3");
    const heading = (h3 ? h3.textContent : "").toLowerCase();
    block.style.display = heading.indexOf(os) >= 0 ? "" : "none";
  });
}
...
async function tryStart() {
  ...
  try {
    ...
    if (result && result.ok === false) {
      log("The agent could not start the bridge: " + (result.error || "unknown error"), "bad");
    ...
    log("The bridge did not come online. It probably is not installed on this PC yet.", "bad");
    showSteps();
    btn.disabled = false;
  } catch (error) {
    log("Could not reach the native agent: " + error.message, "bad");
    log("This PC has not been set up yet. Do the one-time step below, then click Retry.", "muted");
    showSteps();
    btn.disabled = false;
  }
}
...
async function retry() {
  ...
  setPill(false);
  log("Still not running. Make sure you launched it with the steps below.", "bad");
  showSteps();
}
